import React from 'react'
import SearchIcon from "../assets/icons/icons8-search-50.png"

const Header = () => {
  return (
    <div className='grid gap-4 grid-cols-3 m-4'>
      <div className='flex'>
        <img  className='w-14' alt = "menu" src = "data:image/png;base64,"/>
         <img />
      </div>
      <div className='w-44'>
       <input type = "text" placeholder='Search' className='p-2'/>
        <button>Seach</button>
      </div>
      <div>
        <img src = "https://www.iconpacks.net/icons/2/free-user-icon-3296-thumb.png" alt = "user" className='w-14'/>
      </div>
    </div>
  )
}

export default Header